...
locals {
  ...
  # vm_onboot      = local.vm_exists ? data.netbox_virtual_machines.base_vm.vms[0].boot : null
...
resource "cloudflare_record" "vm_dns" {
  zone_id = "7ccf096b817b34b42fc2555c37d3d84c"
  name    = var.subdomain
  content = var.ip_forward
  type    = "A"
  ttl     = 1
  proxied = false
  comment = "DNS automático creado con Terraform"
